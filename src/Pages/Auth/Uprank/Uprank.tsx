...
function Uprank() {
    ...
    const currentRank =UprankGetData?.current_rank;
      ...
      .map(item => ({
        ...
        count: item.count
          ...
          const rankValidation = () => {
            ...
            const items = Object.values(maxLP);
            const maximumTotalLP = items.reduce((total: number, item: any) => total + item.price, 0);
         ...
         <div className="relative overflow-x-auto mt-5 border rounded-md mb-4">
              <table  id="example" className="display table-auto w-full text-sm text-left rtl:text-right text-black  ">
                              ...
                              UprankGetData.packages.map((item: any, index: number) => (
                                        ...
                                        <td className="px-6 py-3">
                                            {item.combo_product_associate_price}
                                        </td>
                                        <td className="px-6 py-3">
                                        ...
                                        <HiOutlineMinusSmall
                                          onClick={() => updateCart(item.id, item.combo_product_associate_price, -1 ,item.combo_product_lp)}
